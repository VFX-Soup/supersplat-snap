...
class Camera extends Element {
    ...
    // intersect a SPECIFIC splat at the given normalized screen coordinate (0-1).
    // Unlike intersect(), this ignores other splats' depth — essential when splats
    // overlap and you need to pick a point on one particular splat.
    async intersectSplat(target: Splat, x: number, y: number) {
        this.picker.prepareDepth(target);
        const normalizedDepth = await this.picker.readDepth(x, y);
        if (normalizedDepth === null) {
            return null;
        }

        const linearDepth = normalizedDepth * (this.far - this.near) + this.near;
        ...
        return { splat: target, position, distance: t };
    }
